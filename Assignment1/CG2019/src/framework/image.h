...
public:
	...
	// applies an algorithm to every pixel in an image 
	// you can use lambda syntax: img.forEachPixel( [](int x, int y, Color c) { return ...; });
	// or callback sintax:   img.forEachPixel( mycallback ); //the callback has to be Color mycallback(int x, int y,Color c) { ... }
	template <typename F>
	Image& forEachPixelXY(F callback){
		for (int x = 0; x < width; x++)
			for (int y = 0; y < height; y++)
				setPixel(x, y, callback(x, y, getPixel(x, y)));
		return *this;
	}
